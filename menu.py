...
class Menu:
    ...
    def run(self, screen, clock, dop_event = None, last_passed_level = pre_start(), level_events = [], level_structure = [], return_player = False, inc_time = 1,dop_event_level = [], i = 0):
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if new_game_button.rect.collidepoint(mouse_pos):
                        ...
                        if dop_event and level_events == []:
                            return "start"
                        game_result = self.start_the_game(screen, clock, level_events, level_structure, return_player, inc_time, dop_event_levels= dop_event_level, i= i)
                        if level_events != []:
                            return game_result
                    elif continue_button.rect.collidepoint(mouse_pos) and self.last_passed_level != 0:
                        if dop_event and level_events == []:
                            return "continue"
                        game_result = self.continue_the_game(screen, clock, level_events, level_structure, return_player, inc_time, dop_event_levels= dop_event_level, i= i)
                        if level_events != []:
                            return game_result
    ...
    def start_the_game(self, screen, clock, level_events, level_structure, return_player = False, inc_time = 1, zero_level=0, dop_event = None, test=0, test_result="",dop_event_levels = [], i =0):
        dop_event2 = None
        test = 0
        if level_structure == []:
        ...
        while result == "running":
            screen.fill('grey')
            if dop_event_levels and i < len(dop_event_levels):
                test_event = dop_event_levels[i]
                i=i+1
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        if dop_event:
                            return "pause"
                        if dop_event_levels and i < len(dop_event_levels):
                            dop_event2 = dop_event_levels[i]
                            i=i+1
                        if i == len(dop_event_levels):
                            test = 1    
                        if test == 1:
    ...
    def continue_the_game(self, screen, clock, level_events, level_structure, return_player = False, inc_time = 1, test=False, dop_event_levels = [], i =0):
        self.start_the_game(screen, clock, level_events, level_structure, return_player, inc_time, zero_level=self.last_passed_level, dop_event_levels= dop_event_levels, i= i)
